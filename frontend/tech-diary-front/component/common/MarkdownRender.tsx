...
const MarkdownRenderBlock = styled.div<{ fontSize?: string, padding?: string }>`
...
function MarkdownRender({ markdown, fontSize, padding }: MarkdownRenderProps) {
  const html = fillter(
    remark()
      .use(remarkParse)
      .use(remarkSlug)
  ...
  return (
    <MarkdownRenderBlock fontSize={fontSize} padding={padding}>
      {element}
    </MarkdownRenderBlock>
  );
}
